...
หลังการบูตครั้งแรก ปลั๊กอินไม่จำเป็นต้องรีสตาร์ตอีกเลย: `npm run build` รีโหลดปลั๊กอินโฮสต์แบบอัตโนมัติผ่าน Cordis HMR และ `npm run build:client` สลับชุดบันเดิลเบราว์เซอร์แบบอัตโนมัติผ่านช่องทาง client-hmr SSE
...
- **ปลั๊กอินโฮสต์** — เปิดรายการ Cordis HMR ใน patch ของโปรไฟล์ โดยให้ watch root ชี้ไปที่ผลลัพธ์ `lib/` ของแพ็กเกจนี้ และคง dependency `link:` ไว้ รัน `npm run build` แล้ว DSH ที่กำลังทำงานจะรีโหลดรายการปลั๊กอินโดยอัตโนมัติ (โพรเซสลูกเซิร์ฟเวอร์ Noema ถูกเริ่มใหม่โดยการรีโหลด) — ไม่ต้องรีสตาร์ตเซิร์ฟเวอร์
...
- **บันเดิลฝั่งไคลเอนต์** — `npm run build:client` เขียน `lib/client.js` ใหม่; ครึ่งโหนดของ client-hmr ทำ stat-poll บันเดิลกราฟทุกรายการ (ค่าเริ่มต้น 500ms) และกระจายเฟรม `rebuilt` ผ่านช่อง SSE `/plugins/events` จากนั้นเบราว์เซอร์สลับโมดูลแบบอัตโนมัติโดยไม่ต้องรีเฟรชหน้า
